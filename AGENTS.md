...
- **UI**: `apps/web/components/TimerPage/TimerPage.tsx` (client) composes pieces in that folder (`TimerScrambleCard`, `TimerSessionAside`, overlays, etc.). **3D `CubeRenderer`** prep rotations are **`[...parseNotation("x2"), ...parseNotation(scramble)]`** — the displayed scramble omits the leading **`x2`**. Scramble line uses **`AlgorithmNotation`** (`centered` prop + `.notationCentered` in SCSS so centering wins over default `.notation`). `TimerScrambleCard` renders the scramble in a bordered bar (cube icon + notation + **copy** action) above the cube stage, and includes **Start** (mobile only) and **Skip scramble** actions.
- **Timer styling**: **`TimerPage.module.scss`** owns every timer surface — gradient `.panel` + `.panelSheen`, the `.eyebrow` caption used as each panel heading (`PanelEyebrow` in `render.tsx`), `.readout` glow, `.scrambleBar`, `.cubeStage` (violet glow + masked floor grid), `.solveChip`, `.statCard`, and the chart chrome. Palette tokens (`--tm-*`) are declared on `.page` for light and overridden under **`html.dark`** + a `prefers-color-scheme: dark` fallback, mirroring `styles/variables.scss` so a stored theme beats the OS setting. `TimerStatsChart` reads `--tm-chart-*` off its host via `getComputedStyle` (canvas cannot use CSS vars) and repaints on a `MutationObserver` watching `documentElement.class`.
...
- **Session aside**: `TimerSessionAside` shows newest solves first (display only; original indices are preserved for selection). The header (**Save session data as...** — `DropdownMenu`: Text/JSON — plus **Clear the session**) stays pinned and only the solve grid scrolls. At **`lg`** the aside is **`position: absolute; inset: 0`** inside a `lg:relative` wrapper (`.asideFill`) so a long solve list cannot stretch the grid row: the row height stays driven by the timer panel and both columns end flush. That rule lives in the SCSS module rather than as a Tailwind `lg:absolute` because `.panel` declares `position` at the same specificity and the module sheet wins on source order. Save/export is disabled when there are no solves.
...
- **Statistics visuals**: `TimerStatsChart` stacks a uPlot line chart (**Solve time over solves**) above a binned bar chart (**Solve time distribution**). Both blocks are `flex: 1`, so when the stats column makes the panel taller the extra height is split between them; uPlot is sized from its host's resolved flex height (safe against the `ResizeObserver`, since that height comes from flex rather than uPlot's own DOM). Histogram gridlines use a 1/2/5 × 10ⁿ tick step (`buildCountTicks`) and bars are scaled against the top tick so they line up with the grid.
...
- **Marketing title typography (shared)**: `apps/web/lib/marketing-hero-title-class.ts` — **`MARKETING_HERO_TITLE_CLASS`** is the exact Tailwind string used on the home hero **`h1`** (“Master the CFOP Method”), the notation hero, and the **running** timer overlay. The **idle** readout no longer uses it: it has its own larger `.readoutTime` in `TimerPage.module.scss`, so changing it does not touch the marketing headings.
